...
class RBAC {
    ...
    // C('USER_AUTH_KEY') = 'userid'  也就是用于存储 用户id
    ...
    /**
     * 权限认证的过滤器方法
     * 验证 $appName 的 当前模块 是否在 $accessList 表中。 也就是说 当前用户是否有对当前模块的访问权限。
     * $accessList 表 根据 C('USER_AUTH_TYPE') 而保存在 session 中，或者在数据库中
     * 
     * 涉及数据：uers_id ，当前模块名，$accessList
     */
     ...
     * @access public
      +----------------------------------------------------------
     * 
     * ease: 参考印象笔记 RBAC getAccessList
     * 返回用户 $authId 的所有有权限的 节点数组 树形结构。
     */
